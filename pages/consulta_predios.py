...
def conectar_bd():
    try:
        conexion = psycopg2.connect(
            host='consultapredios-cqsalazar.j.aivencloud.com',
            database='defaultdb',
            user='avnadmin',
            password='AVNS_7aiHOVksA31tJPEtI6X',
            port='25365'
        )
        return conexion    
...
def load_table(_conexion, option, input):
    ...
    try:
        if df['CONEXION_CARTO'].values[0] is None:
            gdf = gpd.GeoDataFrame(df)
            gdf = gdf.drop(columns=['index'])
            return gdf
        else:
            id_terreno = df['CONEXION_CARTO'].values[0]
            consulta = f"""SELECT * FROM "export_MAESTRO_predio_112025" WHERE "CONEXION_CARTO" = '{id_terreno}' """
            cursor.execute(consulta)
            data = cursor.fetchall()
            columns = [col[0] for col in cursor.description]  # Obtener nombres de columnas
            df = pd.DataFrame(data, columns=columns)
...
if option == 'ID PREDIO':
    ...
    try:
        ...
        if len(selected_df) == 0:
            ...
            id_terreno = selected_df['CONEXION_CARTO'][0]
            ...
            try:
                ...
                #    vecinos = load_manzana(conexion, selected_gdf['CONEXION'][0][:-4])
                    ...
                    vecinos = load_vecino(conexion, option2, selected_df['CONEXION_CARTO'][0])
